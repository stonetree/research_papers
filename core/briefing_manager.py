...
def generate_daily_briefing_manually():
    ...
    prompt = (
        ...
        "【核心抓取关键词与技术方向】：除全局 AI 基础设施与大模型重大突破外，必须重点检索并覆盖以下关键技术方向的最新硬核进展：\n"
        "1. kvcache (KV Cache 显存优化、跨层级卸载/Swapping、量化压缩、PagedAttention、稀疏注意力与长上下文推理加速)\n"
        "2. agent (AI Agent / 智能体系统架构、AIOS、环境交互与动作生成、工具调用、Multi-Agent 协作与自主规划)\n"
        "必须严格聚焦于“工业界与学术界的最新实质性动态”，例如厂商/开源项目发布了具体的新模型、架构或工程实现，或学术界发表了解决突出问题的具体新算法。拒绝宽泛的行业新闻。不要使用你 2025 年之前的内部知识回答。\n"
        "要求：详尽分析，从第一性原理出发，辩证分析问题的正确性、完整性、和必要性，给出遵从科学与事实的结论，生成的报告需具有良好的可读性，结构清晰，突出重点。"
    )
    
    system_instruction = "你是一个顶级的 AI 系统架构师。你在生成报告前必须首先调用 Google Search 工具在全网检索过去 24 小时内最新的技术新闻与论文（重点覆盖 kvcache 优化加速、agent 智能体系统与架构、AI 基础设施前沿）。基于实际检索到的事实，运用第一性原理进行辩证分析。必须确保输出的是最新信息，严禁胡编乱造。"
